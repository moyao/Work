...
import com.golang.management.wigdet.dialog.ImageDialog;
...
public class ImageAdapter extends RecyclerView.Adapter<ImageAdapter.SelectedPicViewHolder> {
    ...
    public void onBindViewHolder(SelectedPicViewHolder holder, final int position) {
        Glide.with(holder.itemView).load(imagesListBeans.get(position).getImage()) .thumbnail(0.2f).placeholder(R.mipmap.load_icon).into(holder.iv_img);
