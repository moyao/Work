package com.golang.management.wigdet;
...
import com.golang.management.utils.DimensUtils;
...
public class NumberProgressBar extends View {
    ...
    public NumberProgressBar(Context context, AttributeSet attrs) {
        ...
        // 构造器中初始化数据
        smallR = DimensUtils.dip2px(context, 4);//小圆半径
        bigR = DimensUtils.dip2px(context, 8);//大圆半径
        radius = DimensUtils.dip2px(context, 10) / 2;//进度条高度
        jR = DimensUtils.dip2px(context, 6);//矩形
    ...
    private void initData() {
        ...
        paintStart.setStrokeWidth(DimensUtils.dip2px(context, 1));
        paintStart.setDither(true);
        paintStart.setAntiAlias(true);
        paintStart.setStyle(Paint.Style.FILL);

        // 已完成进度条画笔的属性
        paintInit.setColor(context.getResources().getColor(R.color.color_FFE457));
        paintInit.setStrokeWidth(DimensUtils.dip2px(context, 1));
        ...
        // 大圆画笔
        paintEndBig.setColor(context.getResources().getColor(R.color.color_FFE457));
        paintEndBig.setAntiAlias(true);
        paintEndBig.setStyle(Paint.Style.FILL);


        // 百分比文字画笔的属性
        int paintTextSizePx = DimensUtils.sp2px(context, 11);  //设置百分比文字的尺寸
        paintText.setColor(context.getResources().getColor(R.color.color_FFE457));
